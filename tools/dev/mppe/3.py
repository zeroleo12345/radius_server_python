...
def _create_plain_text(key) -> bytes:
    ...
    while (len(key) + 1) % 16:
        key += b'\0'
    return chr(key_len).encode() + key


def _create_salt():
    return chr(128 + random.randrange(0, 128)).encode() + chr(random.randrange(0, 256)).encode()
...
if __name__ == "__main__":
    ...
    import base64
    print('recv_key: ' + base64.b64encode(recv_key).decode())
    print('send_key: ' + base64.b64encode(send_key).decode())
